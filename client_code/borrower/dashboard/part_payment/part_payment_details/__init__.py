from ._anvil_designer import part_payment_detailsTemplate
from anvil import *
import anvil.server
...
class part_payment_details(part_payment_detailsTemplate):
  ...
  def button_foreclose_click(self, **event_args):
    ...
    if self.total_payments_made >= self.min_months:
      open_form(
        "borrower.dashboard.foreclosure_request.borrower_foreclosure.foreclose",
        selected_row=selected_row,
        total_payments_made=self.total_payments_made,
      )
    else:
      alert(
        "You are not eligible for foreclosure! You have to pay at least "
        + str(self.min_months)
        + " months."
      )
