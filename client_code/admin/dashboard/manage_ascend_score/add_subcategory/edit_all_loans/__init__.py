...
class edit_all_loans(edit_all_loansTemplate):
  ...
  def save_click(self, **event_args):
    ...
    if self.selected_row is not None:
      ...
      existing_min_points = [row["min_points"] for row in app_tables.fin_admin_ascend_categories.search(group_name="all_loans")]
      max_points = max(existing_min_points + [updated_points])

      existing_group_row  = app_tables.fin_admin_ascend_groups.get(group_name="all_loans")
