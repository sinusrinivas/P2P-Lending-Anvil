from ._anvil_designer import part_payment_copyTemplate
from anvil import *
import anvil.server
...
class part_payment_copy(part_payment_copyTemplate):
  def __init__(self, loan_details, selected_row, **properties):
    ...
    emi_row = app_tables.fin_emi_table.get(loan_id=loan_id, emi_number=emi_number + 1)
  ...
  def pay_now_click(self, **event_args):
    ...
    loan_id = self.loan_details["loan_id"]
    emi_number = self.loan_details["current_emi_number"]
    emi_row_1 = app_tables.fin_emi_table.get(loan_id=loan_id, emi_number=emi_number)
    emi_row = app_tables.fin_emi_table.get(loan_id=loan_id, emi_number=emi_number + 1)
    ...
    if emi_row and emi_row["payment_type"] == "part payment":
      ...
      borrower_wallet = app_tables.fin_wallet.get(
        customer_id=self.loan_details["borrower_customer_id"]
      )
      lender_wallet = app_tables.fin_wallet.get(
        customer_id=self.loan_details["lender_customer_id"]
      )
      ...
      if borrower_wallet and lender_wallet:
        ...
        if borrower_wallet["wallet_amount"] >= text_amount:
          ...
          remaining_amount = (
            float(self.loan_details["remainining_amount"]) - text_amount
          )
          loan_id = self.loan_details["loan_id"]
          ...
          if emi_payment_type in ["One Time", "Monthly", "Three Months", "Six Months"]:
            if emi_payment_type == "Monthly":
              ...
              if self.loan_details["tenure"]:
                ...
                next_next_payment = schedule_payment + timedelta(
                  days=30 * self.loan_details["tenure"]
                )
          else:
      ...
      if entered_amount <= total_emi_amount:
        ...
        if borrower_wallet is not None and lender_wallet is not None:
          ...
          if borrower_wallet["wallet_amount"] >= entered_amount:
            ...
            # Update remaining amount in loan details table
            remaining_amount = (
              float(self.loan_details["remainining_amount"]) - entered_amount
            )
            loan_id = self.loan_details["loan_id"]
              ...
              if emi_payment_type in [
                ...
                if emi_payment_type == "Monthly":
                  ...
                  if self.loan_details["tenure"]:
                    # next_scheduled_payment = prev_scheduled_payment + timedelta(days=30 * tenure)
                    # next_next_payment = self.selected_row['next_payment'] + timedelta(days=30 * tenure)
                    next_scheduled_payment = prev_scheduled_payment + timedelta(
                      days=30 * self.loan_details["tenure"]
                    )
                    next_next_payment = self.selected_row["next_payment"] + timedelta(
                      days=30 * self.loan_details["tenure"]
                    )
              else:
                # Default to monthly calculation
        ...
        alert(
          "Entered amount exceeds the total EMI amount. Please enter a valid amount."
        )
  ...
  def calculate_additional_fees(self, emi_row):
    ...
    days_elapsed = self.calculate_date_difference(
      part_payment_date, datetime.now().date()
    )
    print(days_elapsed)
    ...
    if lapsed_settings:
      lapsed_start = lapsed_settings[
        "minimum_days"
      ]  # Assuming column1 stores the start day
      ...
      if lapsed_start <= days_elapsed <= lapsed_end:
        ...
        total_additional_fees += days_elapsed * (
          lapsed_fee_percentage * float(self.loan_details["emi"]) / 100
        )
    ...
    if default_settings:
      ...
      if default_start <= days_elapsed <= default_end:
        ...
        if default_fee_percentage != 0:
          default_fee = days_elapsed_for_default * (
            default_fee_percentage * float(self.loan_details["emi"]) / 100
          )
        else:
          default_fee = days_elapsed_for_default * default_fee_amount
        total_additional_fees += default_fee
    ...
    if npa_settings:
      ...
      if npa_start <= days_elapsed <= npa_end:
        ...
        if npa_fee_percentage != 0:
          npa_fee = days_elapsed_for_npa * (
            npa_fee_percentage * float(self.loan_details["emi"]) / 100
          )
        else:
          npa_fee = days_elapsed_for_npa * npa_fee_amount
        total_additional_fees += npa_fee
        print(f"NPA Fee: {npa_fee}")
