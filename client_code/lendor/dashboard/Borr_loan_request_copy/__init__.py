...
import anvil
from ._anvil_designer import Borr_loan_request_copyTemplate
from anvil import *
...
# In Borr_loan_request class
class Borr_loan_request_copy(Borr_loan_request_copyTemplate):
  def __init__(self, selected_row, **properties):
    # Set Form properties and Data Bindings.
    self.selected_row = selected_row
    ...
    # Fetch additional details from the 'borrower' table
    try:
      user_request = app_tables.fin_borrower.get(
        customer_id=int(selected_row["borrower_customer_id"])
      )
      if user_request is not None:
        bank_acc_details = user_request["bank_acc_details"]
        borrower_since = user_request["borrower_since"]
        ...
        # Fetch additional details from the 'loan_details' table
        try:
          loan_details = app_tables.fin_loan_details.get(
            loan_id=str(selected_row["loan_id"])
          )
          if loan_details is not None:
            # Assuming 'interest_rate' and 'min_amount' are valid column names in the 'loan_details' table
            interest_rate = loan_details["interest_rate"]
        ...
        self.label_member_since.text = "N/A"
        self.label_bank_acc_details.text = (
          "No data for bank_acc_details in user_request"
        )
    except anvil.tables.TableError as e:
      self.label_member_since.text = "N/A"
      self.label_bank_acc_details.text = f"Error fetching user details: {e}"
    ...
    if user_profile:
      ...
      try:
        ...
        closed_loans = app_tables.fin_loan_details.search(
          borrower_customer_id=borrower_customer,
          loan_updated_status=q.any_of(
            q.like("close%"), q.like("Close%"), q.like("closed loans%")
          ),
        )
  ...
  def calculate_first_emi_due_date(
    self, emi_payment_type, loan_disbursed_timestamp, tenure
  ):
    if emi_payment_type == "Monthly":
      first_emi_due_date = (loan_disbursed_timestamp + timedelta(days=30)).date()
      ...
      if tenure:
        # Add the tenure in months to the loan_disbursed_timestamp
        first_emi_due_date = (
          loan_disbursed_timestamp + timedelta(days=30 * tenure)
        ).date()
      else:
        # Handle the case where tenure is not provided (raise an exception or set to None)
        first_emi_due_date = None
  ...
  def loan_disbursment_btn_click(self, **event_args):
    ...
    # Call the server-side function
    signal, time_difference_seconds = anvil.server.call(
      "loan_disbursement_action", selected_row, email, lender_accepted_timestamp
    )

    # Check the signal and perform actions accordingly
    if signal == "insufficient_balance":
      alert(
        "Warning: Your account balance is insufficient. Please deposit amount into your wallet. If not done within the next 30 minutes, the opportunity may be lost"
      )
      open_form(
      ...
      alert(
        "The designated time has passed. The loan has moved to the 'Lost Opportunities' status."
      )
      open_form("wallet.wallet")
    elif signal == "pay_to_borrower":
      alert("Pay to Borrower")
      ...
      if loan_row:
        ...
        first_emi_due_date = self.calculate_first_emi_due_date(
          emi_payment_type, loan_disbursed_timestamp, tenure
        )
      ...
      transfer_money(
        lender_id=self.lender_customer_id,
        borrower_id=entered_borrower_customer_id,
        transfer_amount=loan_amount,
      )
  ...
  def link_1_click(self, **event_args):
    open_form(
      "lendor_registration_form.dashboard.view_borrower_loan_request.payment_details_view_loan_request",
      selected_row=self.selected_row,
    )
