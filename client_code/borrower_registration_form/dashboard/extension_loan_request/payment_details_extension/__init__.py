...
#                     #     print('last_paid_emi_number - month',last_paid_emi_number - month)
...
class payment_details_extension(payment_details_extensionTemplate):
    ...
    def load_payment_details(self, selected_row):
        ...
        for month in range(1, total_tenure + 1):
            ...
            try:
                payment_date = self.calculate_payment_date(selected_row, month, last_paid_emi_number)
                loan_id = selected_row['loan_id']
                emi_number = payment_number_counter
                emi_row = app_tables.fin_emi_table.get(loan_id=loan_id, emi_number=emi_number)
                ...
                # Handle missed payments for "Three Month" and "Six Month" EMIs
                if month <= last_paid_emi_number:
                    if selected_row['emi_payment_type'] in ('Three Month', 'Six Month'):
                        # Calculate the difference between current month and last paid month
                        month_difference = month - last_paid_emi_number
                        # Adjust the month for payment calculation based on the difference
                        adjusted_month = month if month_difference % ((selected_row['tenure'] / 3) if selected_row['emi_payment_type'] == 'Three Month' else (selected_row['tenure'] / 6)) == 0 else 0
                        emi = self.calculate_scheduled_payment(selected_row['loan_amount'], monthly_interest_rate, (total_tenure - adjusted_month))
                        total_payment = emi
                    else:
                        emi = self.calculate_scheduled_payment(selected_row['loan_amount'], monthly_interest_rate, selected_row['tenure'])
                        total_payment = emi
                ...
                print(f"An error occurred while calculating payment details for month {month}: {str(e)}")
            else:
                # Save the payment details for the month
                payment_details.append({
                    'payment_date': formatted_payment_date,
                    'emi': emi,
                    'total_payment': total_payment,
                    'scheduled_payment_made': scheduled_payment_made,
                    'account_number': account_number
                })

                # Increment the payment number counter
                payment_number_counter += 1
    ...
    def calculate_payment_date(self, selected_row, month, last_paid_emi_number):
        # Implement your logic to calculate the payment date
        # This might involve adding months to a start date
        # or some other business logic specific to your application
        
        # For example, you could start with the loan disbursed timestamp
        # and add months based on the month parameter
        loan_disbursed_timestamp = selected_row['loan_disbursed_timestamp']
        if loan_disbursed_timestamp:
            # Add months based on the month parameter
            payment_date = loan_disbursed_timestamp + timedelta(months=month)
            return payment_date
        else:
            return None

    def calculate_scheduled_payment(self, loan_amount, monthly_interest_rate, tenure):
        # Implement your logic to calculate the scheduled payment
        # This would typically involve using a formula to calculate
        # the EMI (Equated Monthly Installment) based on loan amount,
        # interest rate, and tenure.
        
        # For example, you could use the formula for EMI calculation
        # EMI = [P * r * (1 + r)^n] / [(1 + r)^n - 1]
        # where P is the loan amount, r is the monthly interest rate,
        # and n is the number of months (tenure)
