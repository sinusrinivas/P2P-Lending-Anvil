        ...
        for month in range(1, total_tenure + 1):
            ...
            if month <= last_paid_emi_number:
                ...
                if selected_row['emi_payment_type'] == 'Monthly':
                    ...
                    if month % 3 == 1:  # Calculate only for the first month of the three-month period
                        emi = self.calculate_scheduled_payment(selected_row['loan_amount'], monthly_interest_rate, 3)
                    else:
                        emi = 0  # No payment for the second and third months in the three-month period
                elif selected_row['emi_payment_type'] == 'Six Month':
                    if month % 6 == 1:  # Calculate only for the first month of the six-month period
                        emi = self.calculate_scheduled_payment(selected_row['loan_amount'], monthly_interest_rate, 6)
                    else:
                        emi = 0  # No payment for the months other than the first month in the six-month period
