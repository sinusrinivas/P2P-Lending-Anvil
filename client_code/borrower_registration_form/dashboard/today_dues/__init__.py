...
class today_dues(today_duesTemplate):
    def __init__(self, **properties):
        ...
        if all_loans:
            ...
            loan_id = latest_loan['loan_id']
            loan_details_row = app_tables.fin_loan_details.get(loan_id=loan_id)
            user_profile = app_tables.fin_user_profile.get(customer_id=loan_details_row['lender_customer_id'])
            if loan_details_row is not None and user_profile is not None:
                loan_amount = loan_details_row['loan_amount']
                scheduled_payment = latest_loan['scheduled_payment']
                next_payment = latest_loan['next_payment']
                ...
                # # Get lender's full name if available
                # if 'lender_full_name' in loan_details_row:
                #     lender_full_name = loan_details_row['lender_full_name']
                # else:
                #     lender_full_name = "N/A"
        ...
        if not loan_details:
            ...
            for loan_due in all_loans_due:
                ...
                product_description = loan_due['product_description']
                borrower_customer_id = loan_due['borrower_customer_id']
                lender_full_name = loan_due['lender_full_name']
