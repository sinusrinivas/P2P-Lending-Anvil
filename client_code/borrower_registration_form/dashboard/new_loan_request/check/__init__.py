...
class check(checkTemplate):
    def __init__(self, product_group, product_cat, product_name, loan_amount, tenure_months, user_id, interest_rate, processing_fee,
                 membership_type, product_id, product_desc, total_repayment_amount, credit_limit, emi_payment_type, total_interest=None, processing_fee_amount=None, entered_values=None,
                 emi=None, **properties):
        ...
        self.dropdown_payment_numbers.placeholder = "Select the Instalment"
    ...
    def display_one_time_payment_details(self):
        ...
        # Populate dropdown with PaymentNumber values
        payment_numbers = [payment['PaymentNumber'] for payment in payment_details]
        self.dropdown_payment_numbers.items = payment_numbers
        self.label_30.visible = False
        self.dropdown_payment_numbers.visible = False
        self.repeating_panel_2.items = self.payment_details
    ...
    def display_monthly_payment_details(self):
        ...
        # Populate dropdown with PaymentNumber values
        payment_numbers = [str(payment['PaymentNumber']) for payment in payment_details]
        self.dropdown_payment_numbers.items = payment_numbers
    ...
    def display_three_month_payment_details(self, total_interest_amount):
        ...
        self.payment_details = payment_schedule

        # Populate dropdown with PaymentNumber values
        payment_numbers = [str(payment['PaymentNumber']) for payment in payment_schedule]
        self.dropdown_payment_numbers.items = payment_numbers
    ...
    def dropdown_payment_numbers_change(self, **event_args):
        selected_payment_number = self.dropdown_payment_numbers.selected_value
        if selected_payment_number:
            # Filter payment details based on selected payment number
            filtered_payment_details = [payment for payment in self.payment_details if payment['PaymentNumber'] == int(selected_payment_number)]
            # Update repeating panel with filtered payment details
            self.repeating_panel_2.items = filtered_payment_details
        else:
            alert("Please select a payment number.")
